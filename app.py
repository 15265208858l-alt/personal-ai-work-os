...
st.subheader("个人 AI 工作操作系统 V2.1")
...
if st.button("🚀 开始执行", type="primary"):
    ...
    # 识别到A股后，强制进入专业股票Agent。
    if (
        stock_code
        and result.get("module") == "investment"
        and result.get("agent") != "gold_agent"
    ):
    ...
    if effective_route.get("agent") == "value_stock_agent":
        ...
        st.success(
            f"✅ ValueStock AI分析完成：{vr.get('name', '未知')}（{vr.get('code', stock_code)}）"
        )
        ...
        with st.expander("💰 查看三大报表关键数据", expanded=False):
            ...
            for col, label, key in report_items:
        ...
        percentile = val_get(
            hist,
            "percentile",
            default=val_get(val, "historical_percentile", default=None),
        )
        historical_level = val_get(val, "historical_level", default="数据不足")
        st.write(
            f"**历史PE分位：** {'暂无' if percentile is None else percentile}%　｜　"
        ...
        if isinstance(gq, dict):
            st.write(
                f"成长质量：**{gq.get('score', '暂无')}/100**　"
                f"等级：**{gq.get('level', '暂无')}**"
            )
