...
def execute_tasks(tasks, user_request, route_result=None, **kwargs):
    ...
    opportunity_result = cockpit_result = industry_stock_result = None
    finance_error = None
    ...
    if agent == "value_stock_agent":
        try:
            ...
            value_stock_result = {"success": False, "error": f"ValueStock AI 调用异常：{type(exc).__name__}: {exc}"}
    ...
    if results and agent == "finance_intelligence_agent":
        ...
        results[0]["industry_stock_result"] = industry_stock_result
        results[0]["finance_error"] = finance_error
        results[0]["message"] = "财经情报 → 投资机会 → 决策驾驶舱 → 行业/A股候选 已完成。"
    return results
