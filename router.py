...
INVESTMENT_WORKFLOW = [
    "行业与成长空间", "企业护城河", "营收与净利润成长", "ROE及盈利能力",
    "经营现金流与利润匹配", "资产负债表与偿债能力", "应收账款与存货质量",
    "商誉、资本开支及潜在减值", "管理层、股东结构、关联交易", "估值与买入价格"
]

INVESTMENT_RISKS = [
    "经营现金流与净利润是否匹配", "应收账款增长是否过快", "存货是否异常增加",
    "资产负债率是否过高", "短期偿债压力", "商誉减值风险", "资本开支压力",
...
STOCK_NAME_HINTS = [
    "美的集团", "特变电工", "紫金矿业", "牧原股份", "紫光股份", "博敏电子",
    "沪电股份", "深南电路", "生益科技", "平安银行", "章源钨业", "步步高"
]

KEYWORDS = {
    "file": ["pdf", "word", "excel", "ppt", "文件", "报告", "合同", "表格"],
    "investment": ["股票", "a股", "美股", "港股", "黄金", "白银", "投资", "估值", "roe", "pe", "pb", "公司", "股票池", "建仓", "重仓"],
    "finance": ["财经", "新闻", "美联储", "美债", "美元", "原油", "宏观", "地缘政治", "经济", "cpi", "非农", "降息", "加息"],
...
def classify_task(task):
    if not task:
        return "unknown"
    if is_stock_value_investment_task(task):
        return "investment"
    text = task.lower()
    scores = {module: 0 for module in KEYWORDS}
...
def analyze_investment_task(task):
    text = task.lower()
    result = {"workflow": INVESTMENT_WORKFLOW, "risk_scan": INVESTMENT_RISKS, "type": "投资分析"}
    if "黄金" in text:
