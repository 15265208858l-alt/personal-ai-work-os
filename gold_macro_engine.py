# =========================================================
# Personal AI Work OS — Gold Macro Decision Engine V4.0
# =========================================================
# 核心：黄金、美元、名义利率、实际利率、Fed代理、PCE、就业、地缘政治、ETF代理、技术位、情景推演
# 原则：缺失数据不强行打分；代理指标明确标注；结论区分“方向”和“位置”。
...
HEADERS = {"User-Agent": "Mozilla/5.0 Personal-AI-Work-OS/4.0"}

SYMBOLS = {"gold": "GC=F", "dxy": "DX-Y.NYB", "us10y": "^TNX", "us2y": "^UST2Y", "gld": "GLD"}
...
def _fred(series_id, limit=120):
    r = requests.get(FRED.format(series_id), headers=HEADERS, timeout=10)
    r.raise_for_status()
...
def _trend(rows):
    if not rows or len(rows) < 60:
        return "数据不足"
    a = sum(v[1] for v in rows[-10:]) / 10
    b = sum(v[1] for v in rows[-30:]) / 30
    c = sum(v[1] for v in rows[-60:]) / 60
    x = rows[-1][1]
...
def _cpi(data):
    rows = _months(data)
    if not rows:
        return {"index": None, "mom_pct": None, "yoy_pct": None}
    latest = float(rows[0]["value"]); prev = float(rows[1]["value"]) if len(rows) > 1 else None
    yoy = (latest / float(rows[12]["value"]) - 1) * 100 if len(rows) >= 13 else None
...
def _geo(headlines):
    if not headlines:
        return {"score": None, "level": "数据不足", "reason": "暂无足够新闻样本"}
    risk = ["war", "strike", "attack", "missile", "sanction", "conflict", "tension", "iran", "israel", "russia", "ukraine"]
    ease = ["ceasefire", "truce", "peace", "de-escalation", "talks", "agreement"]
    signal = 0
    for row in headlines:
        text = row.get("title", "").lower(); signal += sum(1 for x in risk if x in text); signal -= sum(1 for x in ease if x in text)
    score = max(0, min(100, 50 + signal * 5))
    return {"score": score, "level": "高" if score >= 70 else "低" if score <= 30 else "中", "reason": "避险风险偏高" if score >= 70 else "风险偏低" if score <= 30 else "风险中性偏复杂"}


def _technical(rows):
    closes = [x[1] for x in rows]
    last = closes[-1]
    s20, s60 = min(closes[-20:]), min(closes[-60:]); r20, r60 = max(closes[-20:]), max(closes[-60:])
    ma20 = sum(closes[-20:]) / 20; ma60 = sum(closes[-60:]) / 60
    return {"price": last, "support_short": s20, "support_mid": s60, "resistance_short": r20, "resistance_mid": r60, "ma20": ma20, "ma60": ma60, "distance_from_20d_high_pct": (last / r20 - 1) * 100 if r20 else None, "distance_from_20d_low_pct": (last / s20 - 1) * 100 if s20 else None}
...
def _weighted(parts):
    ...
    score = round(sum(s * w for s, w, _ in valid) / total)
    reasons = [r for _, _, r in sorted(valid, key=lambda x: abs(x[0] - 50), reverse=True) if r][:8]
    return score, round(total), reasons


def _scenario(score, trend, real10y, dxy20, technical):
    up, flat, down = 35.0, 40.0, 25.0
    if score is not None:
        up += (score - 50) * 0.65; down -= (score - 50) * 0.40
    ...
    up = max(5, min(80, up)); down = max(5, min(80, down)); flat = max(5, 100 - up - down)
    total = up + flat + down
    return {"上涨/延续": round(up / total * 100), "震荡/高位消化": round(flat / total * 100), "回撤/转弱": round(down / total * 100)}
...
def analyze_gold_market() -> dict[str, Any]:
    ...
    with ThreadPoolExecutor(max_workers=16) as pool:
        ...
        for i, query in enumerate(NEWS_QUERIES): jobs[pool.submit(_retry, lambda q=query: _news(q), 2)] = ("news", f"news_{i}")
    ...
    if not gold:
        return {"success": False, "agent": "gold_agent", "version": "V4.0", "error": "黄金价格数据暂时无法获取。", "diagnostics": errors, "elapsed_seconds": round(time.time() - started, 2)}
    ...
    trend_score = 75 if trend == "偏强" else 25 if trend == "偏弱" else 50 if trend == "震荡" else None
    ...
    score, confidence, reasons = _weighted([
        ...
        (real_score, 15, f"实际10Y约{real10y:.2f}%" if real10y is not None else "实际利率数据不足"),
        (rate_score, 10, f"利率环境：{rate_label}" if rate_score is not None else "利率预期数据不足"),
        (pce_score, 10, f"核心PCE约{core_pce:.2f}%" if core_pce is not None else "PCE数据不足"),
        ...
        (tech_score, 10, "技术趋势偏强" if trend == "偏强" else "技术趋势偏弱" if trend == "偏弱" else "技术趋势震荡"),
        (etf_score, 5, "GLD价格动量为ETF情绪代理" if etf_score is not None else "ETF代理数据不足"),
    ])
    outlook = "偏多" if score is not None and score >= 68 else "偏空" if score is not None and score <= 35 else "震荡" if score is not None else "数据不足"
    scenario = _scenario(score, trend, real10y, dxy20, technical)
